...
//    random_seed(true);
    ...
    {
        sample_out.activation = random_activation(iseed);
        sample_out.zero_to_one = random_0to1(iseed);
        sample_out.in_range = random_in_range(iseed, 3, 11);
        if (print_option == 1)
        {
            fprintf(stdout, "\t%f \t%f \t%f\n", sample_out.activation, sample_out.zero_to_one, sample_out.in_range);
        }
...
//    }
    if (read_option == 1)
    {
        ...
        {
            fprintf(stdout, "\t%f \t%f \t%f\n", sample_in.activation, sample_in.zero_to_one, sample_in.in_range);
        }
        fclose(file_in);
    }
    return 0;
